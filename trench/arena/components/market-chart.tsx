...
export function MarketChart({ data, currentValue, color = "#60A5FA", type = "YES" }: MarketChartProps) {
  ...
  // Ensure we have data
  if (!dataWithFlags || dataWithFlags.length === 0) {
    return (
...
export function generateChartData(
  ...
  for (let i = intervals; i >= 0; i--) {
    const timeLabel = `${intervals - i}h ago`
    ...
    let value = Math.max(0.1, Math.min(99.9, currentValue + variation))

    // For NO charts, invert the value (100 - YES value) with appropriate variation
    if (type === "NO") {
      value = 100 - value
    }
