...
export default function MarketPage() {
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      const newComment = createComment(marketPdaStr, walletAddress, commentInput, undefined, isHolder)
  ...
  const handlePostReply = async (parentId: string) => {
    ...
    try {
      const marketPdaStr = market.marketPda.toString()
      const isHolder = Boolean((userYesPosition?.amount && userYesPosition.amount > 0n) || (userNoPosition?.amount && userNoPosition.amount > 0n))
      const newReply = createComment(marketPdaStr, walletAddress, replyText, parentId, isHolder)
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start gap-3 mb-1">
                        ...
                        <div className="relative w-8 h-8 rounded-full overflow-hidden bg-[#0B0B0D] border border-white/10 flex-shrink-0 mt-0.5">
                          <img
                            src={`https://api.dicebear.com/7.x/identicon/svg?seed=${market.tokenMint}`}
                            alt="Token"
