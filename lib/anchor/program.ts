...
async function fetchIdl(): Promise<any> {
  ...
  idlFetchPromise = (async () => {
    try {
      console.log("🔍 Fetching IDL from /idl.json...")
      const response = await fetch('/idl.json')

      console.log("📡 IDL fetch response:", {
        ok: response.ok,
      ...
      console.log("📄 Raw IDL text length:", text.length)
      console.log("📄 Raw IDL text preview:", text.substring(0, 200) + "...")
      ...
      console.log("🔄 Parsed raw IDL:", raw)

      // Handle wrapped IDL (some tools wrap it in { idl: {...} })
      const idl = raw.idl ?? raw
      console.log("🎯 Final IDL object:", idl)
...
export async function getProgram(connection: Connection, wallet: anchor.Wallet): Promise<anchor.Program<any>> {
  ...
  console.log("🔍 Final IDL validation check:", {
    hasVersion: !!idlCopy.version,
    hasName: !!idlCopy.name,
    hasInstructions: Array.isArray(idlCopy.instructions),
    version: idlCopy.version,
    name: idlCopy.name,
    instructionCount: idlCopy.instructions?.length,
    metadataVersion: idlCopy.metadata?.version,
    metadataName: idlCopy.metadata?.name
  })
  ...
  if (!idlCopy.version || !idlCopy.name || !Array.isArray(idlCopy.instructions)) {
    console.error("❌ IDL validation failed:", {
      version: idlCopy.version,
      name: idlCopy.name,
      instructionsType: typeof idlCopy.instructions,
      instructionsIsArray: Array.isArray(idlCopy.instructions)
    })
  ...
  console.log("FINAL IDL CHECK (before Anchor):", {
    ...
    accountCount: idlCopy.accounts?.length,
      hasInstructions: Array.isArray(idlCopy.instructions),
    instructionCount: idlCopy.instructions?.length,
    accounts: idlCopy.accounts?.map(acc => ({ name: acc.name, size: acc.size })),
  })
  console.log("Full accounts array:", idlCopy.accounts)
  }
